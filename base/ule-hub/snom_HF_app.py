...
def handle_dev_registered(client, msg):
    ...
    # update deletes the old device and replaces it with the new one
    # we need to get the new device
    final_device = hf_devices.get_device_by_id(new_device.device_id)
    print('####################################################')
    print('correct interface attributes depending on profile')
    print('apply known profile changes for interfaces')
    print(f'update dev {final_device.device_id}:{final_device.device_name}')
    dev_update_profile_changes(final_device)
    print('####################################################')
...
Stores all the information for each device registered to the hub in HFDevices  
    ...
    print('####################################################')
    # read data from attributes - which attributes are related to cmds cannot be known 
    #device_info(client_handle, ['aa', '12'])
    #print('Create Snom Minibrowser Files ')
    create_minibrowser_ULE(hf_devices)
    print('####################################################')
...
def device_info_snom(client_handle, device_id):
    ...
    if dev != None:
        for u in dev.units:
            if u.unit_id != 0:
                print('####################################')
                print(f'Unit:{u.unit_name} - {u.profile.profile_name}')
                print('####################################')
                for i in u.interfaces:
                    print('====================================')
                    print(f'{i.intrf_name} = {i.intrf_id}')
                    print('====================================')
...
# set a part of an attribute value 
def snom_set_cmd_attribute_value(client_handle, argv):
    # set 9 1 514 1 7 "Hue 0-359" __X__ 
    if len(argv) == 8:
        _, device_id, unit_id, interface_id, _, cmd_id, label, value = argv

        dev = hf_devices.get_device_by_id(int(device_id)) 
        unit = dev.get_unit_by_id(int(unit_id))
        interface = unit.get_interface_by_id(int(interface_id))
        cmd = interface.get_c2s_cmd_by_id(int(cmd_id))
        payload_descrs = descrs = interface.get_c2s_cmd_by_id(int(cmd_id)).payload_descriptions[0]

        # decode backslash from label into single slash, e.g. Degrees/second
        try:
            # send new value to the device
            enc_label = label.replace("%2C", "/")
            # set the new value in local structs
            cmd.payload_descriptions[0].set_attribute_value_by_description(enc_label, int(value))
            # add full data values from idx 6 
            argv.pop()
            argv.pop()
            for idx,x in enumerate(payload_descrs.attribute_values):
                argv.insert(idx+6,x)

            snom_send_cmd(client_handle, argv)
        except:
            log('snom_set_cmd_attribute_value: send failed')
        print(cmd)

def snom_send_generic_cmd(argv):
    #snom_send_cmd(client_handle, argv)
    call = f'{argv[0]}(client_handle, argv)'
    ...
    eval(call)
    # we do not know if the command needs data fetched from DECT.. we wait 
    time.sleep(2.0)

    sys.stdout = old_stdout
    #
    text = f'<pre><code>{mystdout.getvalue()}</code></pre>'
    #text = ''
    return text
...
def snom_send_cmd(client_handle, argv):
    ...
    if len(argv) >= 7:
        ...
        try: 
            ...
            print("deviceID={}, unitID={}, interfaceID={}')".format(device_id,unit_id,interface_id))
            device_id = int(device_id)
            interface_id = int(interface_id)
            ...
            print("deviceID={}, unitID={}, interfaceID={}, msg_type={}, cmdID={}, data={})".format(device_id, unit_id, interface_id, msg_type, cmd_id, user_data))
            logging.exception('number conversion didnt work')
        return
    else:
        return 'cmd not sent.'
    
    # we got user data as bytes 
    try: 
        device_id = int(device_id)
        interface_id = int(interface_id)
        unit_id = int(unit_id)
...
def snom_handle_x0300_report(device_id, unit_id, interface_id, cmd_id, hl):
    log(f"snom_handle_x0300_report: data={hl}")
    handle_Simple_Power_Metering_Interface(device_id, hl)


def snom_handle_x0304_state(device_id: int, unit_id: int, interface_id: int, cmd_id: int, hl: list):
    unit = hf_devices.get_device_by_id(device_id).get_unit_by_id(unit_id)
    interface = unit.get_interface_by_id(interface_id)
    cmd = interface.get_s2c_cmd_by_id(cmd_id)
    snom_handle_airtemp(device_id, unit_id, cmd.cmd_id, cmd.cmd_name)
...
def snom_handle_x0100_state(device_id, unit_id, interface_id, cmd_id, hl):
    ...
    if snom_handle_window_open_close(device_id, proximity):
        log('Window Open Close Detector, action fired')
    else:
        log('Window Open Close Detector, action FAILED')
...
def snom_handle_server_cmds(device_id, unit_id, interface_id, cmd_id, hl):
    try:
        ...
        logging.exception(f'command {cmd_id} for interface {interface_id} is not existing!')
        return
...
def snom_handle_server_attribute_reporting(device_id: int, unit_id: int , interface_id:int , cmd_id: int, hl: List) -> bool:
    global hf_devices
    if interface_id == 6:
        # Attribute Report specified in HF-Service page 77
        data_len = len(hl)
        # report type and report number
        type = (hl[0] & 0x80) >> 7
        report_number = hl[0] & 0x7F
        log('snom_handle_server_attribute_reporting: cmd_id={}'.format(cmd_id))
        log('snom_handle_server_attribute_reporting: report type={}, num:{}'.format(type, report_number))
        hl_idx = 1

        while data_len > hl_idx:
            unit = hl[hl_idx]
            hl_idx += 1
            # report type and reporting interface 
            report_type = hl[hl_idx] & 0x80
            interface_id_t = ((hl[hl_idx] << 8) + hl[hl_idx+1]) & 0x7fff
            hl_idx += 2
            # get interface of given unit
            # if no interface can be found a dummy interface is returned 
            interface = hf_devices.get_device_by_id(device_id).get_unit_by_id(unit).get_interface_by_id(interface_id_t)
            if interface.intrf_name == 'unknown':
                log('Interface in Report not known - abort - skip..')
                return False
            # get number of reported attributes of this interface 
            num_attribs = hl[hl_idx]
            hl_idx += 1
            for attr in range(0, num_attribs):
                attribute_id = hl[hl_idx]
                hl_idx += 1
                # only events have a type like COV, equal
                if type == 1: # Event 
                    # get attribute type and value
                    attribute_type = hl[hl_idx] # 0 = COV
                    hl_idx += 1
                    # find length in bytes of attribute value
                current_attribute = interface.get_attribute_by_id(attribute_id)
                if not current_attribute:
                    log('Interface {}has no attribute={} - abort - skip..'.format(interface_id, attribute_id))
                    return False                
                length_value = current_attribute.attribute_type
                # alternative code to check byte order 
                #value = 0
                #for l in range(0, length_value):
                #    value += (hl[hl_idx] << ((length_value-1-l)*8))
                #    hl_idx += 1
                #attribute_value = value
                # finally set attribute values
                data_slice = hl[hl_idx:hl_idx+length_value]
                current_attribute.attribute_values = data_slice
                log('received report attribute {} on Interface {}.'.format(current_attribute, interface.intrf_name))

                hl_idx += length_value

            # check and run in case we have a server cmd function defined for the interface 
            s2c_cmd = interface.get_s2c_cmd_by_id(cmd_id)
            log('report attribute command {}.'.format(s2c_cmd))

            if s2c_cmd:
                level = int(data_slice[0])
                command_routine = s2c_cmd.cmd_action
                try:
                    eval(command_routine)
                except:
        ...
        logging.exception('not an Attribute Report!')
...
def snom_handle_fun_msg(client, msg):
    ...
    log('Profile: device_id={}, src_unit_idx={}: unit_id={}, profile_type={}, interface={}'.format(device_id, unit_id,
    ...
    if unit_id == 0:
        # Attribute Reporting 
        if message_type == 1 and interface_id == 0x6:
            # Attribute Reporting unit
            log("Device {}: Attribute Reporting".format(device_id))
            try:
                datastr=msg.params["DATA"]
                hl = [int(x, 16) for x in datastr.split(' ')]
            except:
                hl=[]
            log("Attribute data {}".format(hl))
            snom_handle_server_attribute_reporting(device_id, unit_id, interface_id, interface_member, hl)

        if interface_id == 0x0111:
            # RSSI
            log("Device {}: RSSI={}".format(device_id, msg.params["DATA"]))
    ...
    if unit_id != 0:
        ...
        if int(msg.params["DATALEN"]) > 0:
            log(f'data raw #{int(msg.params["DATALEN"])}, {msg.params["DATA"]}')
            datastr=msg.params["DATA"]
            hl = [int(x, 16) for x in datastr.split(' ')]
        else:
            log(f'no data received')
            datastr=''
            hl = []
