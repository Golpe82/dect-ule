...
def return_minibrowser_cmd_payload(dev_id: int, i_id: int, u_id: int, c: HFC2SCommand) -> str:
    ...
    for pd in c.payload_descriptions:
        ...
        # flatten options lists for mini browser until we have a more suitable page type
        att_desc_flat = []
        for element_or_list in pd.attribute_descriptions:
            if type(element_or_list) == list:
                for tuple in element_or_list:
                    att_desc_flat.append(tuple)
            else:
                att_desc_flat.append(element_or_list)
        
        # only work with list of tuples. Not lists of lists of tuples.
        for idx, ad in enumerate(att_desc_flat):
            if len(ad) == 3:
                label, start, end = ad
...
def return_server_attribute_values(dev_id: int, u_id: int, i: HFInterface, u_profile: str) -> tuple:
    # create jinja data 
    data_header = {}
    data_header['DeviceID'] = dev_id
    data_header['InterfaceName'] = i.intrf_name
    data_header['InterfaceId'] = i.intrf_id
    data_header['UnitID'] = u_id
    data_header['BackUrl'] = f'{HTML_URL}/{dev_id}/{u_id}/{i.intrf_id}'

    data_list = []
    sign = False 
    for pd in i.server_attributes:    
        
        for idx, add in enumerate(pd.attribute_descriptions):
            if type(add) != list:
                ...
                d_list = add
            # create tuple or options list of tuples.
            data_tuple_list = []
            for idx, ad in enumerate(d_list):
                if len(ad) == 3:
                    label, start, end = ad
                if len(ad) == 4: # with sign 
                    label, start, end, sign = ad
                label = label.strip()
                # convert data into int
                last_val = pd.get_attribute_value_by_description(label)
                # prepare the option value, if any 

                option_val = 9999
                option_val_s = None
                if label is not None:
                    option_val_s = ([x for x in label.split() if x.startswith('0x')])
                    if type(option_val_s) is list and option_val_s != []:
                        option_val = int(option_val_s[0],0)
                    else:
                        option_val = 999
                    
                data_tuple = (idx, label, start, end, sign, last_val, option_val)
                data_tuple_list.append(data_tuple)
            data_list.append(data_tuple_list)

    return (data_header, data_list)


def return_cmd_options_payload(dev_id: int, i_name: str, i_id: int, u_id: int, c: HFC2SCommand) -> tuple:
    # create jinja data 
    data_header = {}
    data_header['DeviceID'] = dev_id
    data_header['InterfaceName'] = i_name
...
if __name__ == "__main__":
    ...
    # without option lists
    data_tuple = return_cmd_payload(7, 'some name', interface_id, unit_id, interface.get_c2s_cmds()[0])
    print(data_tuple)
    # with option lists
    data_tuple = return_cmd_options_payload(7, 'some name', interface_id, unit_id, interface.get_c2s_cmds()[0])
    print(data_tuple)

    data_tuple = return_server_attribute_values(7, unit_id, interface, 'some profile name')
    print(data_tuple)
